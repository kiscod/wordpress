...
<h1><a href="<?php echo get_permalink($post->post_parent); ?>" title="<?php esc_attr(printf(RETURN_TO, get_the_title($post->post_parent))); ?>" rel="gallery">
                                    <?php
                                    /* translators: %s - title of parent post */
                                    printf(__('<span>&larr;</span> %s', 'blackwell'), get_the_title($post->post_parent));
                                    ?>
                                </a></h1>
</div>
</div>
        ...
        <div class="grid_24">
            <div class="page-content">
                          <div class="fullwidth">
                            ...
                            <?php
                            printf(BY_AUTHOR, 'meta-prep meta-prep-author', sprintf('<a class="url fn n" href="%1$s" title="%2$s">%3$s</a>', get_author_posts_url(get_the_author_meta('ID')), sprintf(esc_attr__('View All Post By', 'blackwell'), get_the_author()), get_the_author()
                            ));
                            ?>
                            <span>|</span>
                            <?php
                            ...
                            if (wp_attachment_is_image()) {
                                ...
                                <p><a href="<?php echo $next_attachment_url; ?>" title="<?php echo esc_attr(get_the_title()); ?>" rel="attachment">
                                        <?php
                                        $attachment_size = apply_filters('twentyten_attachment_size', 1235);
